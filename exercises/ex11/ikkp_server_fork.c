...
/* I can confirm that ikkp_server_fork.c is resilient to segfaults
in it's children processes (the child process dies but the server
doesn't) and that ikkp_server_thread.c is not; when a thread gets a 
segfault, the whole server crashes. */